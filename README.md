...
Anywhere 是一款为 **uTools** 打造的深度定制化 AI 助手插件。它不仅仅是一个聚合 API 的聊天窗口，更是一个集成了 **定时任务调度**、**MCP 工具调用**、**Skill 流程编排**、**多模态交互** 以及 **多端数据同步** 的生产力平台。
...
* **灵活触发**：支持间隔 (Interval)、每日 (Daily)、每周 (Weekly)、每月 (Monthly) 等多种触发方式。
* **无人值守**：配合 MCP 工具，自动执行新闻搜集、系统监控、报表生成等任务，并自动保存结果到本地。
...
* **内置强力工具**：开箱即用，无需配置即可支持 **Python 代码执行**、**全能文件操作** (读/写/搜索/正则替换)、**终端命令执行** (Bash/PowerShell)、**联网搜索** (DuckDuckGo) 以及 **任务管理**。
...
| 模块                   | 说明                                                              | 文档链接                        |
| :--------------------- | :---------------------------------------------------------------- | :------------------------------ |
| **定时任务**     | **(新增)** 创建自动化任务，让 AI 定时执行并生成报告。       | [查看文档](./docs/task_doc.md)     |
