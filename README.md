...
1. **AI Studio（Google免费提供Gemini API，推荐）**
   ...
   * **中转平台使用项目**：https://github.com/Komorebi-yaodong/openai-gemini （直接连接仓库，未进行代码更改，可以通过分析仓库代码进行安全分析，**安全提示**：当你使用他नों的中转API时，他人有能力更改中转API代码实现读取你使用的密钥的，请鉴别使用），中转平台支持 `gemini-2.5-flash-preview-tts`的语音输出模型（使用方法同 `gpt-4o-audio-preview`，也同 `tts-1`两种调用方式，查看openAI官方文档）
...
6. **全新输入框体验**：对话窗口输入框采用垂直布局，**重新聚焦窗口时，输入框会自动激活并恢复上次的光标位置，无缝衔接你的思路**。
...
3. **舒适视觉体验**：
   * **支持全局浅色/暗色主题切换**，为你提供更舒适的阅读与操作环境。
   * **新增用户与AI对话气泡的颜色区分**，上下文更分明，对话流更清晰易读。
...
- **新增**
  - **快捷助手图标下载**：支持下载已设置的快捷助手图标，方便备份与复用。
- **优化**
  - **快捷助手独立配置**：每个快捷助手现在都可以独立配置窗口大小、位置、流式传输、失焦关闭及窗口置顶等行为，实现高度个性化。
  - **智能滚动优化**：优化了对话窗口的滚动体验，在向上浏览历史记录时，新增了便捷的上下文导航按钮，可以快速跳转至上一条/下一条消息或返回底部。
  - **视觉体验提升**：用户与AI的对话气泡增加了颜色区分，使对话流更清晰易读。
  - **输入框聚焦优化**：重新聚焦对话窗口时，输入框光标将自动恢复到上次离开时的位置，而非直接跳转到末尾，无缝衔接输入思路。
- **修复**
  - **输入法兼容性**：修复了在使用中文输入法输入英文时，按回车键可能导致意外发送消息的问题。
  - **配置竞争问题**：修复设置窗口打开时，独立窗口保存配置（窗口大小、位置、缩放）失败的问题。
  - **性能优化**：解决了在删除聊天消息后可能出现的界面卡顿问题，提升了流畅性。

### 2025-07-15

- **新增**
  - **全局主题切换**：在设置页面增加全局浅色/暗色主题切换功能。
  - **全新输入框样式**：对话窗口输入框采用全新设计
