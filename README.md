...
6. **全新输入框体验**：对话窗口输入框采用全新设计，支持水平与垂直两种布局，可在设置中切换。

### 二、沉浸式对话与显示

1. **多轮对话追问**：窗口模式支持持续的对话交流，你可直接输入文字、粘贴图片或拖拽上传文件。通过 `Ctrl+Enter`或 `Enter`发送信息。
2. **丰富内容渲染**：支持Markdown渲染，包括公式、代码高亮、图片重加载及Mermaid图表，让AI回复更美观、易读。代码块现已优化，支持顶部/底部一键复制。
...
5. **自定义UI外观**：支持自定义快捷助手图标及快捷助手分类标签，打造专属的操作界面。设置页面全新卡片式设计，更美观、更易用。
