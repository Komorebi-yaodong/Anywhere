...
*   **添加服务商**：
  ...
  *   填写 `API 密钥` 和 `API URL`。
  *   点击 `从 API 获取模型`，Anywhere 将从该服务商获取可用的模型列表，可以从中**搜索**并**选择**模型添加到模型列表中。
*   **添加自定义模型**：
  *   点击 `手动添加` 按钮，可以手动设置一个自定义模型名称。
*   **多密钥随机轮询**：
...
* **语言**：
  * 选择 Anywhere 应用界面的显示语言，支持中文、英文、日文、俄文（水平有限，AI翻译）。
* **流式响应 (Stream)**：
...
* **数据管理**
  ...
  * **WebDAV**
    * 配置 WebDAV 同步，将当前设置同步到远程服务器，注意保证**WebDAV 配置路径**与**WebDAV 数据路径**不同，分别用于存储配置文件与对话数据文件
* **窗口尺寸**
  * *此功能主要在提示词的 `显示模式` 选择 `独立窗口` 时有效。*
...
* **`新增快捷助手` (添加提示词)**：位于底部左侧，点击即可新建一个提示词。
  * **`图标`**：快捷助手图标，方便查找。由于Utools数据库限制，图片大小请尽可能小，建议尺寸：64x64。
  * **`快捷助手名称` (提示词名称)**：
  ...
  * **`类型` (调用类型)**：定义该提示词的触发方式。
    * `通用`【默认】：当框选文本或截图后，呼出 Utools 菜单，Anywhere 会将选中文本、截图、文件作为输入。
    * `文本`：**仅在选择文本后**，呼出 Utools 菜单，Anywhere 会自动将选择的文本作为输入发送给AI。
    * `截图`：**仅在截图后**，呼出 Utools 菜单，Anywhere 会自动将选择的截图作为输入发送给AI。
      ![调用类型示例](image/t6.png)
    * `文件`：**选中文件**，呼出 Utools 菜单，Anywhere 会自动将选择的文件（图片文件也是该类型）为输入发送给AI（选中保存的对话将会直接打开之前的对话）。
  * **`显示模式` (展示模式)**：定义AI回复结果的显示方式。
    * `输入框`：AI的输出结果会直接显示在当前聚焦的输入框中。这使得AI能够直接协助进行文本输入或内容补全。
      * *请确保在调用此功能时，的光标正聚焦在一个可编辑的输入框中。*
    * `剪切板`：AI的输出结果会被直接复制，进入剪切板。
    * `独立窗口`【默认】：AI的输出结果将以一个独立的窗口形式展示。此模式支持多轮对话追问。
  * **`模型` (AI模型)**：设置执行该提示词所使用的具体AI模型。默认为第一个服务提供商的第一个模型 。
  * **模型参数**
    * `流式响应`【默认开启】：若启用，AI的回复将以流式（即文字逐个显示）方式展现，提供更流畅的交互体验。
    * `自定义温度`【默认关闭】：不同情境下启用不同温度，可以调整AI的回答风格，默认不更改官方参数。
  * **快捷助手参数**
    * `文本/截图直接发送`【默认开启】
      * 若启用，划词、截图后调用快捷助手，将直接发送内容，无需点击发送
      * 若关闭，划词、截图后调用快捷助手，可以进行内容补充再发送
    * `文件直接发送`：【默认关闭】
      * 若启用，选中文件后调用快捷助手，将直接发送内容，无需点击发送
      * 若关闭，选中文件后调用快捷助手，可以进行内容补充再发送
    * `补充文本信息/自动添加事件`：【默认关闭】
      * 若启用，在独立窗口模式下，将自动添加当前时间以文本内容形式发送给AI，以获取更准确的回复，同时满足部分提供商（如vertax不允许仅仅发送图片而不附带文本内容）
  * **`快捷助手内容` (系统提示词)**：可以在此处设置AI的系统提示词。AI将根据设定的提示词要求进行回复，例如限定角色、语气或回答格式。
* **新增标签**：用于对提示词进行分类与批量管理

### 4. 云端会话页面

当且仅当配置WebDAV时，该页面才可用。用于管理云端保存的对话记录，可以恢复对话、删除对话记录等。

![云端会话页面](image/s0.png)
*图示：显示云端保存的会话名称、修改时间、会话文件大小与可执行操作*

* **操作**
  * `聊天`：打开保存的会话，继续聊天，对话加载速度由多种因素决定（WebDAV连接传输速率、文件大小）
  * `重命名`：重命名保存的会话
  * `删除`：删除保存的会话
