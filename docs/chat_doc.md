...
欢迎使用 **Anywhere** 智能助手。本模块旨在帮助您高效管理、备份及同步您与 AI 的所有对话记录。无论是日常聊天还是**定时任务**生成的执行报告，都可以在这里统一查阅。通过强大的本地与云端（WebDAV）协同机制，您可以确保数据在多设备间无缝流转。
...
* **定义**：存储在您当前设备硬盘上的对话记录。
* **存储机制**：
  1. **实时保存**：Anywhere 会每隔 **15秒** 自动将**当前活跃且已命名**的对话保存到您指定的本地目录中。
