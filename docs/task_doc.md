...
**定时任务** 是 Anywhere 2.0 引入的重磅功能。它允许您将 AI 的工作自动化，让 Anywhere 成为您的 24 小时全天候智能员工。
...
Anywhere 内置了 **Task Manager** MCP 服务。这意味着您可以创建一个 AI 助手，让它来帮您管理任务！

**示例 Prompt：**

> "帮我创建一个每天早上8点执行的任务，名字叫'早安问候'，内容是给我讲一个笑话。"

**AI 的操作：**

1. AI 调用 `create_task` 工具。
2. 自动填入参数：`name="早安问候"`, `schedule_type="daily"`, `time_param="08:00"`, `instruction="讲一个笑话"`。
3. 任务创建成功并立即生效。
...
> **A:** 很可惜不会执行。Anywhere 是基于 uTools 运行的插件，依赖于主进程存活。如果电脑休眠或 uTools 完全退出，计时器将暂停。唤醒后，如果错过了触发时间，系统会根据逻辑判断是否补发（通常会忽略过期的触发，等待下一个周期）。
