...
def get_summary():
    ...
    text = [
        ...
        window_text,
        "不论你进行如何修改，一定保证不会破坏已有的功能，前端修改一定要保持相同的主题风格，并保证节省开发者工作量的原则，对于javascript，如果只改动一个函数，请给出完整的函数代码并告诉我在哪里进行覆盖，对于vue代码，请给出所需要修改的对应的块（完整的<script setup>块、<template>块或者<style>块等完整的代码），如果只修改部分函数，就直接直接告诉我在哪里进行怎样的修改就好了，不用给出全部代码，千万不要省略代码\n\n"
    ]
